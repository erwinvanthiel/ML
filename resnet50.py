...
index = 3
rotations = [0, 270, 0, 0]
photos = ["beach", "puddle", "lizard", "beer"]

# Momentum Induced Fast Gradient Sign Method 
def mi_fgsm(model, images, loss_function=torch.nn.CrossEntropyLoss(), eps=0.02, device='cpu'):
    ...
    for i in range(iters):    
        ...
        target = torch.zeros(outputs.shape).float()
        target[0][924] = 1
        model.zero_grad()
        cost = L(outputs, target)
        cost.backward()
...
test_image = Image.open('images/{0}.jpg'.format(photos[index])).rotate(rotations[index])

input_tensor = image_transform(test_image)

input_batch = input_tensor.unsqueeze(0)

# If available, move the model and input tensor to GPU for faster computation
...
# Load the class labels for CIFAR-10
IMAGENET_LABELS = [
    ...
    "earthstar", "hen-of-the-woods", "bolete", "ear", "toilet paper"
]

# Define the images and captions
adv = mi_fgsm(model, input_batch)

pred1 = IMAGENET_LABELS[torch.argmax(model(input_batch)).item()]
pred2 = IMAGENET_LABELS[torch.argmax(model(adv)).item()]

input_tensor = input_tensor.permute(1, 2, 0)
adv = adv.squeeze().permute(1, 2, 0)
perturbation = (adv - input_tensor) * 50

images = [input_tensor] #, perturbation, adv]
captions = ['prediction: {0}'.format(pred1), 'perturbation x 50', 'adversarial example: {0}'.format(pred2)]
